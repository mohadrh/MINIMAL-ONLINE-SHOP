...
/* ============================================================
   کدِ اینماد در عنوانِ صفحه — موقت

   ⚠ بعد از «تایید عنوان» در پنلِ اینماد، این خط را خالی کن:

       const ENAMAD_TITLE = '';

   بعد ‎npm run build:host && npm run pack:site‎ و دوباره آپلود.
   همین یک خط، هیچ جای دیگری دست نمی‌خورد.

   ⚠ و چرا باید حتماً برداشته شود

   عنوانِ صفحه همان چیزی است که در نتیجه‌ی گوگل و در پیش‌نمایشِ
   لینک در تلگرام و اینستاگرام دیده می‌شود. تا وقتی این عدد
   آن‌جاست، هر کسی لینکِ فروشگاه را ببیند یک کدِ بی‌معنا کنارِ
   اسم می‌بیند — و روی سایتی که لینکش دست‌به‌دست می‌شود، این
   هزینه‌ی واقعی است.

   دو راهِ دیگرِ اینماد (فایلِ ‎10189409.txt‎ در ریشه و متاتگِ
   ‎enamad‎ پایین‌تر) ماندگارند و برای بازدیدکننده نامرئی. آن دو
   می‌مانند؛ فقط همین یکی موقتی است.
   ============================================================ */
const ENAMAD_TITLE = '10189409';

const BASE_TITLE = 'فونیکس شاپ — اشتراک هوش مصنوعی، گیم و شماره مجازی';
const TITLE = ENAMAD_TITLE ? `${BASE_TITLE} ${ENAMAD_TITLE}` : BASE_TITLE;
const DESCRIPTION =
...
export const metadata: Metadata = {
  ...
  /* ============================================================
     ...
     اینماد برای تأییدِ مالکیتِ دامنه سه راه می‌دهد و هر کدام
     کافی است. دو تای اول را برداشتیم:

       ۱ فایلِ خالی در ریشه → ‎public/10189409.txt‎
       ۲ همین متاتگ در صفحه‌ی اصلی

     راهِ سوم «گذاشتنِ کد در عنوانِ صفحه» بود که عمداً *انجام
     نشد*: عنوانِ صفحه همان چیزی است که در نتیجه‌ی گوگل و در
     پیش‌نمایشِ لینک در تلگرام دیده می‌شود. عوض کردنش یعنی تا
     وقتی تأیید بیاید، هر کسی لینکِ سایت را ببیند یک عددِ
     بی‌معنا می‌بیند. دو راهِ دیگر هیچ چیزی را برای بازدیدکننده
     عوض نمی‌کنند.

     ⚠ این متاتگ بعد از گرفتنِ نماد هم می‌ماند.

     وسوسه‌اش هست که بعدش برش داریم. ولی اینماد هر چند وقت
     دامنه را دوباره چک می‌کند و نبودنش یعنی نماد معلق می‌شود —
     درست همان وقتی که کسی حواسش نیست. یک خط متاتگ هزینه‌ای
     ندارد.
     ============================================================ */
  other: {
    enamad: '10189409',
